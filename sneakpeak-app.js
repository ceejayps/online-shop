const homelist = document.querySelector('#first-o');
var entry = [];
...
function renderHomelist(doc) {
    ...
    homelist.appendChild(parentz);

};

db.collection('prods').where("sneak", "==", true).get().then((snapshot) => {
    snapshot.docs.slice(0, 8).forEach(doc => {
        entry.push({ doc })
        renderHomelist(doc);
